           ...
           Word16 old_lsp[])  /* (i) Q15: old lsp                */
{
   ...
   for (i = 1, j = LPCO; i <= (LPCO/2); i++, j--) {
      ...
      fa_exp[i] = norm_l(a0);
      fa_man[i] = round30To16(L_shl(a0, fa_exp[i]));  // Q(8+fb_exp[i])

      a0 = L_mult0(pc[i], 4096);     // Q24
      a0 = L_msu0(a0, pc[j], 4096);  // Q24
      a0 = L_add(a0, L_shr(L_deposit_h(fb_man[i-1]),fb_exp[i-1]));  // Q24
      fb_exp[i] = norm_l(a0);
      fb_man[i] = round30To16(L_shl(a0, fb_exp[i]));  // Q(8+fb_exp[i])
   }
                 ...
                 Word16 nd2)     /* (i): order                       */
{
   ...
   b_exp[1] = norm_l(a0);
   b_man[1] = round30To16(L_shl(a0, b_exp[1]));                            // b[1] = c[nd2-1] + t * b[0]
   ...
   for (i=2;i<nd2;i++){
      ...
      b_exp[i] = norm_l(a0);
      b_man[i] = round30To16(L_shl(a0, b_exp[i]));                         // b[i] = c[nd2-i] - b[i-2] + t * b[i-1]
   }

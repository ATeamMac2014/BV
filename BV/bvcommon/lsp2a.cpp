           ...
           Word16 a[])      /* (o) Q12 : predictor coefficients (order = 10)  */
{
   ...
   a[1] = round30To16(L_shl(a0,4));    // Q12 - includes 0.5 factor of a[1] = 0.5*(p[1]+q[1])
   for(i=1, n=2; i<LPCO; i++, n++){
      a0 = L_add(p[i], p[n]);    // Q23
      a0 = L_add(a0, q[n]);      // Q23
      a0 = L_sub(a0, q[i]);      // Q23
      a[n] = round30To16(L_shl(a0,4)); // Q12 a[n] = 0.5 * (p[i] + p[n] + q[n] - q[i]);
   }
                        ...
                        Word16 *lsp)   /* Q15 */
{
   ...
   for(n=1; n<=(LPCO>>1); n++) { 
      ...
      coslsp = add(costable[index], round30To16(L_shl(a0, 6)));            // Q15 cos((double)PI*lsp[2*n-2])

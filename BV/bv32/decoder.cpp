                 ...
                 Word16 *x)
{
   ...
   for (ssf=0;ssf<2;ssf++) {
      ...
      gainq = round30To16(L_shl(a0, gain_exp)); /* Q2 w.r.t gain_exp */
   ...
   /* normalize 32-bit excitation and round30To16 to 16-bit */
   for(i=0; i<FRSZ; i++)
      qv[i] = round30To16(L_shl(qv32[i], new_exp)); /* Q0 normalized by new_exp */
   ...
   /* shift output back */
   for(i=0; i<FRSZ; i++)
      x[i] = round30To16(L_shr(L_deposit_h(x[i]),new_exp));

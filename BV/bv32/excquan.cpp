             ...
             )
{
   ...
   for (j=0; j<CBSZ; j++) {
      *sp3 = shr(*sp2++,1);				/* Q-1 by gain_exp */
      ...
      for (n=1; n<VDIM; n++) {
         ...
         *sp4 = t = round30To16(a0);					/* Q-1 gain_exp */
   ...
   for (m = 0; m < SFSZ; m += VDIM) {
      ...
      for (n = m; n < m + VDIM; n++) {
         ...
         *sp3++ = round30To16(L_shl(L_sub(a0,*lp4++),gexpm3));
      ...
      for (n = m; n < m + VDIM; n++) {
         ...
         ltnfm[MAXPP1+n] = round30To16(L_shl(a1,1)); /* Q1 */
         
         /* CALCULATE QUANTIZED LPC EXCITATION VECTOR qv[n] */
         a1 = L_add(a2, *lp2++); /* Q16 */
         
         /* UPDATE LONG-TERM PREDICTOR MEMORY */
         ltsym[MAXPP1+n] = d[n] = round30To16(L_shl(a1,1)); /* Q1 */
